...
class SecureEntry:
    def __init__(self, id, parent_id, name, hint, hint_resp, data, parent_pvc: PyVaultCrypto) -> None:
        # DataFrame id
        self.id = id

        # Entry Name, e.g. Debit Card
        # Hint to Unlock
        ...
        # Hint Response Hash
        self._pvc = PyVaultCrypto(hint_resp)
        self.hint_resp_hash, self.hint_resp_hash_salt = self._pvc.get_pwd_hash()
    ...
    def _decryt(self) -> Tuple(str, str, str, dict):
    ...
    def add_child(self, child: SecureEntry):
        # Decrypt to get child_dict
        name, hint, data, child_dict = self._decryt()
